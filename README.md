...
Проєкт вже має налаштований GitHub Actions workflow для автоматичного деплою на GitHub Pages. Щоб активувати публікацію:

1. **Перейдіть до Settings вашого репозиторію на GitHub**
   - Відкрийте `https://github.com/BeletskyiBohdan/SAIT-photo/settings`

2. **Налаштуйте GitHub Pages**
   - У лівому меню знайдіть розділ **"Pages"**
   - У секції **"Build and deployment"**:
     - **Source**: Виберіть **"GitHub Actions"**
   - Натисніть **"Save"** (якщо потрібно)

3. **Дочекайтеся автоматичного деплою**
   - Перейдіть на вкладку **"Actions"** у вашому репозиторії
   - Ви побачите запущений workflow "Deploy to GitHub Pages"
   - Зачекайте, поки він завершиться (зелений чекмарк ✓)

4. **Відкрийте ваш сайт**
   - Після успішного деплою сайт буде доступний за адресою:
   - **https://beletskyibohdan.github.io/SAIT-photo/**

### Автоматичне оновлення

Кожного разу, коли ви пушите зміни до гілки `main`, GitHub Actions автоматично:
- Встановить залежності
- Зробить білд проєкту (`npm run build`)
- Опублікує оновлену версію на GitHub Pages

### Перевірка статусу деплою

Якщо щось пішло не так:
1. Перейдіть на **Actions** вкладку
2. Знайдіть останній workflow run
3. Перегляньте логи для виявлення помилок

### Важливо

- Файл `vite.config.js` вже налаштований з `base: '/SAIT-photo/'` для правильної роботи на GitHub Pages
- Переконайтеся, що у Settings > Actions > General дозволено "Read and write permissions" для GITHUB_TOKEN
